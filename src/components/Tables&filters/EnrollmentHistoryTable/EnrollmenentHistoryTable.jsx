...
export default function EnrollmentHistoryTable({
  ...
  const handleEnableAccess = (id) => {
    setSelectedId(id);
    setConfirmMessage("هل أنت متأكد من تفعيل الوصول لهذا المستخدم؟");
  ...
  const handleDisableAccess = (id) => {
    setSelectedId(id);
    setConfirmMessage("هل أنت متأكد من إلغاء الوصول لهذا المستخدم؟");
  ...
  const enableAccess = async (id) => {
    try {
      ...
      if (response.ok && result?.[0].status === "success") {
        setResultMessage(result.msg || "تم تفعيل الوصول بنجاح");
        setShowResultModal(true);
  ...
  const disableAccess = async (id) => {
    try {
      ...
      if (response.ok && result?.[0].status === "success") {
        setResultMessage(result.msg || "تم إلغاء الوصول بنجاح");
        setShowResultModal(true);
        // Refresh data to reflect changes
        fetchData(currentPage);
      } else {
  ...
  const trainingData = filter.map((item, index) => {
    ...
    return {
      ...
      columns: [
        ...
        {
          type: "user",
          name: item.buyer.full_name || item.full_name || "N/A",
          id: item.buyer.id || item.id || "N/A",
        },
        {
          type: "user",
          name: item.webinar.creator.full_name || item.full_name || "N/A",
          id: item.webinar.creator.id || item.id || "N/A",
        },
        {
          type: "user",
          name: item.webinar.translations?.[0].title || item.full_name || "N/A",
          id: item.webinar.translations?.[0].id || item.id || "N/A",
        },
        {
          type: "label",
          value: item.manual_added ? t("manual") : t("automatic"),
        },
        { type: "text", value: item.created_at || item.join_date || "N/A" },
        { type: "label", value: item.status_label || "N/A" },
        {
          type: "actionbutton",
          label: t("actions"),
          action: () => {
