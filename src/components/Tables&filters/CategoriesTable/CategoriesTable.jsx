...
export default function CategoriesTable({ dat }) {
  ...
  const getSubcatData = async (Itemid) => {
    try {
      ...
      viewStack.current.push({ data, subcat });
    ...
    try {
      ...
      if (result.errors) {
        ...
        const updatedItem = {
          ...
          ...dataa,
          subCategoriesCount: dataa.sub_categories
            ? dataa.sub_categories.length
            : 0,
        };
        setData((prev) =>
          prev.map(
  ...
  const handleBack = () => {
    ...
    if (prev) {
      setData(prev.data);
      setSubcat(prev.subcat);
    } else {
      // fallback to initial props if stack empty
      setData(dat);
      setSubcat(false);
    }
    // optional: clear details state
    setReqTable([]);
    setSub([]);
  };
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        {(subcat || showModal) && (
          <button className="btn custfontbtn" onClick={handleBack}>
            {t("back")}
          </button>
        )}
      </div>
