  ...
  return <>
      <div className=' d-flex  flex-column-reverse w-100'>
          {courses.map((course , key) => {
              return <div  key={key} className=' d-flex flex-column mt-3  ' >
                  <div className='bg-white-color '>
                      <div className='bg-white-color d-flex justify-content-around  align-items-center p-3 '>
                          <Image src={course[5] === "مدفوع" ?  paid : circle } alt='circle' />
                          <h4 className=' h8' > {course[1]} </h4>
                          <Image src={open && index === key ? up : down} alt='icon' onClick={() => { toggle(key) }} />
                      </div>
                  </div>
      {  open && index === key ? 
                  <div className=' mt-3'>
